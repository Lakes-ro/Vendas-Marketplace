/**
 * ONBOARDING.JS v1.1
 * v1.1 (auditoria): botão "❓" e cartão de Missões não ficam mais em cima
 *    da barra lateral (ajuste no style.css); tour do vendedor com
 *    o nome certo da aba (CONFIGURAÇÕES DA LOJA).
 ...
 *    "❓" flutuante (canto inferior esquerdo, ao lado da barra lateral).
...
const Onboarding = {
    ...
    BUYER_STEPS: [
        ...
        },
        {
            ...
            text: 'Use a barra de busca no topo, ou toque numa categoria (🍎 Alimentos, 🥤 Bebidas...) pra filtrar rapidinho. No celular, arraste o dedo pra ver mais categorias.'
        },
        {
            icon: '❤️',
    ...
    SELLER_STEPS: [
        ...
        {
            icon: '💳',
            title: 'CONFIGURAÇÕES DA LOJA — sua Chave Pix',
            text: 'Cadastre sua chave Pix aqui (pode ter mais de uma) — é ela que aparece pro comprador pagar direto pra você. Sem isso preenchido, a venda acontece do mesmo jeito, mas trava até vocês combinarem tudo pelo WhatsApp.',
            cta: { label: 'Ver CONFIGURAÇÕES DA LOJA', tab: 'vendor-settings' }
        },
        {
            icon: '🟢',
        ...
        },
        {
            ...
            text: 'Você recebe um aviso na hora quando alguém compra um produto seu (com o site aberto). Quando o Pix cair na sua conta, confirme o recebimento no BI, em "Detalhamento de Pedidos".'
        }
    ],
    ...
    _renderMissionsWidget() {
        try {
            ...
            if (auth?.isSupreme?.() && !auth.hasSellerTools?.()) {
                document.getElementById('onboarding-missions-widget')?.remove();
                return;
            }
            const role = window.APP?.auth?.role === 'seller' ? 'seller' : 'buyer';
