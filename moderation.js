    ...
    _renderQueue() {
        ...
        container.innerHTML = this.queue.map(p => {
            ...
            const imageHtml = p.image_url
                ? `<img src="${p.image_url}" alt="${esc(p.name)}" class="w-24 h-24 object-cover rounded-xl flex-shrink-0" onerror="this.style.display='none'">`
            ...
            const actionButtons = isFlagged ? `
                <div class="flex gap-2 mt-3">
                    <button onclick="window.APP.moderation.approve('${p.id}')" class="flex-1 py-2 text-xs font-bold bg-green-600/20 hover:bg-green-600/30 text-green-400 rounded-lg transition-all">
                        ✅ Aprovar e Publicar
                    </button>
                    <button onclick="window.APP.moderation.block('${p.id}')" class="flex-1 py-2 text-xs font-bold bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded-lg transition-all">
                        🚫 Manter Bloqueado
                    </button>
                </div>
            ` : `
                <div class="flex gap-2 mt-3">
                    <button onclick="window.APP.moderation.block('${p.id}')" class="w-full py-2 text-xs font-bold bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded-lg transition-all">
    ...
    _renderKeywords() {
        ...
        container.innerHTML = this.keywords.map(k => `
            <span class="inline-flex items-center gap-1.5 bg-white/5 border border-white/10 text-slate-300 text-xs font-semibold px-3 py-1.5 rounded-full">
                ${esc(k.word)}
                <button onclick="window.APP.moderation.removeKeyword('${k.id}')" class="text-red-500 hover:text-red-400 font-black" aria-label="Remover">✕</button>
            </span>
        `).join('');
    },
