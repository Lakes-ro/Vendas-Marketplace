 ...
 * ✅ v2.0: gestão de Administradores Supremos — o Admin Supremo pode
 ...
 * ✅ v2.1 FIX CRÍTICO: o arquivo tinha um bloco inteiro de código colado
 *    por engano em cima do original — uma cópia mais ANTIGA e menos
 *    segura de várias funções (sem window.escapeHtml() nos nomes/emails,
 *    e sem a interface de conceder/revogar "Fechar a Loja Inteira"), que
 *    ficava por baixo, mas por estar definida DEPOIS no objeto,
 *    "vencia" e sobrescrevia a versão boa. Também faltava a função
 *    toggleCloseStorePermission() — o botão "Conceder/Revogar" da
 *    permissão de fechar a loja existia na tela mas chamava uma função
 *    que nunca tinha sido escrita, e por isso não fazia nada. Ambos
 *    corrigidos: o arquivo agora só tem uma versão de cada função (a
 *    correta, com escapeHtml), e o botão de conceder/revogar funciona.
 */
...
const Tenants = {
    ...
    async loadDashboard() {
        try {
            ...
            // 4. Carregar e renderizar Admins Supremos
                ...
                return `
                    <div class="bg-slate-900/50 p-6 rounded-2xl border border-white/5 hover:border-blue-500/30 transition-all">
                        ...
                        <div class="flex justify-between items-start mb-4">
                            <div class="flex-1">
                                <h3 class="text-lg font-black text-white">${window.escapeHtml(seller.full_name)}</h3>
                                <p class="text-xs text-slate-400 mt-1">${window.escapeHtml(seller.email)}</p>
                                <p class="text-xs text-slate-500 mt-1">📱 ${window.escapeHtml(seller.phone || 'Sem telefone')}</p>
                            </div>
            ...
            statsDiv.innerHTML = `
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-gradient-to-br from-green-900/30 to-green-800/10 border border-green-500/30 p-4 rounded-2xl">
                        ...
                        <div class="text-[10px] text-slate-600 mt-1">
                            ${topSeller ? `Top: ${window.escapeHtml(topSeller.name)}` : 'Sem dados'}
                        </div>
                    </div>
            ...
            if (tenantProducts.length === 0) {
                ...
                content.innerHTML = tenantProducts.map(p => `
                    <div class="flex justify-between items-center bg-slate-800/50 p-3 rounded-lg border border-white/5">
                        <div class="flex-1">
                            <span class="text-sm font-bold text-white">${window.escapeHtml(p.name)}</span>
    ...
    // ============================================================
    // GESTÃO DE ADMINISTRADORES SUPREMOS
    // ============================================================
    ...
    async loadSupremeAdmins() {
        try {
            const { data, error } = await _supabase
                .from('profiles')
                .select('id, email, full_name, is_founder, can_close_store, created_at')
                .eq('role', 'supreme')
    ...
    renderSupremeAdmins() {
        ...
        // Só a conta fundadora vê os botões de conceder/revogar
        const viewerIsFounder = !!window.APP?.auth?.profile?.is_founder;

        list.innerHTML = this.supremeAdmins.map(admin => {
            const isFounder = !!admin.is_founder;
            const isSelf = admin.id === currentUserId;
            const canClose = !!admin.can_close_store;

            const closeStorePermissionHtml = isFounder ? `
                <div class="text-[10px] text-yellow-400 font-bold uppercase mt-1">🔒 Fechar loja: sempre (fundador)</div>
            ` : `
                <div class="flex items-center gap-2 mt-1 flex-wrap">
                    <span class="text-[10px] font-bold uppercase ${canClose ? 'text-red-400' : 'text-slate-500'}">
                        ${canClose ? '🔴 Pode fechar a loja inteira' : '⛔ Não pode fechar a loja'}
                    </span>
                    ${viewerIsFounder ? `
                        <button onclick="window.APP.tenants.toggleCloseStorePermission('${admin.id}', ${canClose})" class="text-[10px] font-bold text-blue-400 hover:text-blue-300 underline">
                            ${canClose ? 'Revogar' : 'Conceder'}
                        </button>
                    ` : ''}
                </div>
            `;
                ...
                <div class="flex justify-between items-center bg-white/5 p-4 rounded-xl border border-white/5">
                    <div class="flex-1 min-w-0">
                        <span class="font-bold text-white block">
                            ${window.escapeHtml(admin.full_name || 'Sem nome')}
                        ...
                        <span class="text-xs text-slate-500 truncate block">${window.escapeHtml(admin.email)}</span>
                        ${closeStorePermissionHtml}
                    </div>
                    ${!isFounder ? `
    ...
    /**
     * ✅ NOVO (v2.1): liga/desliga a permissão can_close_store de um
     * Admin Supremo que não é o fundador. O botão que chama isso já
     * existia na tela, mas a função nunca tinha sido escrita — clicar
     * não fazia nada. O banco também garante (via gatilho) que só a
     * conta fundadora consegue de fato realizar essa troca, então essa
     * checagem aqui é a primeira camada, não a única.
     */
    async toggleCloseStorePermission(userId, currentlyCan) {
        const action = currentlyCan ? 'revogar' : 'conceder';

        if (!confirm(`Deseja ${action} a permissão de "Fechar a Loja Inteira" para esta conta?`)) return;

        try {
            const { error } = await _supabase
                .from('profiles')
                .update({ can_close_store: !currentlyCan })
                .eq('id', userId);

            if (error) throw error;

            log(`✅ Permissão ${currentlyCan ? 'revogada' : 'concedida'}`, 'success');
            await this.loadSupremeAdmins();
        } catch (err) {
            log(`❌ Erro ao alterar permissão: ${err.message}`, 'error');
            alert(`❌ Erro: ${err.message}`);
        }
    },
