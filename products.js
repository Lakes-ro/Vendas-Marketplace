...
const Products = {
    ...
    async fetchAll() {
        try {
            ...
            await this._attachVendorOnlineStatus(); // ✅ marca produtos de vendedor offline
     ...
     * carregados, e anexa `p.vendor_online` em cada produto. Produto sem
     * linha em vendor_status é tratado como disponível (mesmo padrão do
     * banco: is_online default = true).
     */
    async _attachVendorOnlineStatus() {
        try {
            const ownerIds = [...new Set(this.products.map(p => p.owner_id).filter(Boolean))];
            ...
            this.products.forEach(p => { if (p.vendor_online === undefined) p.vendor_online = true; });
        }
    },
    ...
    render() {
        try {
            ...
            grid.innerHTML = this.products.map(p => {
                ...
                return `
                    <div class="bg-slate-900/40 p-6 rounded-[32px] border border-white/5 flex flex-col gap-4 hover:border-blue-500/30 transition-all ${!vendorOnline ? 'opacity-60' : ''}">
                        ${p.image_url
