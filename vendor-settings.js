/**
 * VENDOR-SETTINGS.JS v2.0
 * ✅ Usa Supabase (vendor_status / vendor_status_history) — não existe mais
 *    dependência de /api/vendor/status (que nunca existiu no server.js)
...
const VendorSettings = {
    ...
    async toggleStatus() {
        try {
            ...
            // Recarrega produtos — o status do vendedor pode afetar visibilidade/compra
    ...
    startScheduleCheck() {
        ...
        this._scheduleInterval = setInterval(() => {
            ...
            const shouldBeOnline = currentTime >= this.openingTime && currentTime < this.closingTime;

            if (shouldBeOnline !== this.currentStatus) {
                this.toggleStatus();
            }
        }, 60000);
    },
