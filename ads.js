...
const Ads = {
        ...
        container.innerHTML = requests.map(req => `
            <div class="bg-slate-900/50 p-4 rounded-xl border border-orange-500/20">
                ...
                <div class="flex gap-2">
                    ...
                    <button onclick="window.APP.ads._rejectRequest('${req.id}')" class="flex-1 py-2 text-xs font-bold bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded-lg transition-all">
                        ❌ Rejeitar
                    </button>
                </div>
            </div>
    ...
    async _rejectRequest(requestId) {
        ...
        try {
            ...
            log('✅ Solicitação rejeitada', 'success');
            await this._loadAdminRequests();
        } catch (err) {
            log(`❌ Erro ao rejeitar: ${err.message}`, 'error');
            alert(`❌ Erro: ${err.message}`);
        }
    },
